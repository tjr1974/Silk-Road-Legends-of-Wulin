...
class Player extends Character {
  ...
  weapons; // Add a property to hold player's weapons
  constructor(uid, name, bcrypt) {
    ...
    this.weapons = new Set(); // Initialize weapons Set
  }
  importConfig() {
    try {
  ...
  getInventoryCapacity() {
    return this.server.config.INVENTORY_CAPACITY; // Use config to return maximum inventory capacity
  }
  ...
  attackNpc(target) {
    const { actions } = this; // Destructure properties
  ...
  incrementFailedLoginAttempts() {
    ...
    if (this.consecutiveFailedAttempts >= 3) {
      ...
      this.server.gameManager.disconnectPlayer(this.uid); // Disconnect player from the game
    }
  }
  showInventory() {
  ...
  describeCurrentLocation() {
    const describeLocationManager = new DescribeLocationManager(this);
    describeLocationManager.describe(); // Call the describe method
  }
  lookAt(target) {
    const lookAtManager = new LookAt(this); // Instantiate LookAt with the player instance
    lookAtManager.look(target); // Call the look method
  }
  addWeapon(weapon) {
    if (weapon instanceof WeaponItem) {
      this.weapons.add(weapon); // Add weapon to player's weapons
      MessageManager.notifyPickupItem(this, weapon.name); // Notify weapon pickup
    }
  }
  removeWeapon(weapon) {
    this.weapons.delete(weapon); // Remove weapon from player's weapons
  }
  static async createNewPlayer(name, age) { // New static method to create a player
    return new CreateNewPlayer(name, age); // Use CreateNewPlayer to create a new instance
  }
}
// Health Regenerator ****************************************************************************
/*
...
class HealthRegenerator {
  ...
  start() {
    if (!this.regenInterval) {
      this.regenInterval = setInterval(() => this.regenerate(), this.config.REGEN_INTERVAL); // Use config
    }
  }
  regenerate() {
  ...
  getRegenAmountPerMinute() {
    const regenRates = {
      "in combat": this.config.REGEN_RATES.get('IN_COMBAT'), // Use .get() to access values from the Map
      "standing": this.config.REGEN_RATES.get('STANDING'),
      "sitting": this.config.REGEN_RATES.get('SITTING'),
      "sleeping": this.config.REGEN_RATES.get('SLEEPING'),
      "unconscious": this.config.REGEN_RATES.get('UNCONSCIOUS'),
      "meditating": this.config.REGEN_RATES.get('MEDITATING'),
    };
...
class Location {
  ...
  addNpc(npc) {
    this.npcs.add(npc.id); // Add NPC to the location
  }
  addPlayer(player) {
  ...
  getNpcs() {
    return Array.from(this.npcs).map(npcId => this.gameManager.getNpc(npcId)); // Retrieve all NPCs in the location
  }
}
// NPC ********************************************************************************************
/*
...
class Npc extends Character {
  constructor(name, sex, currHealth, maxHealth, attackPower, csml, aggro, assist, status, currentLocation, mobile = false, zones = [], aliases) {
    ...
    this.currHealth; // Changed from #currHealth
  ...
  startMovement() {
    ...
    }, this.CONFIG.NPC_MOVEMENT_INTERVAL); // Set movement interval
  }
  moveRandomly() {
    ...
    const validDirections = Object.keys(location.exits).filter(direction => {
      const isValid = this.zones.includes(location.zone[0]); // Check if the zone matches
      return isValid ? direction : null; // Return direction if valid, otherwise null
    });
